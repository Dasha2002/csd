<!DOCTYPE html>
<html lang="ru">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Главная</title>
    <link rel="stylesheet" href="/css/style.css">
</head>

<body>

    <?php include __DIR__ . '/includes/header.php'; ?>
    <?php include __DIR__ . '/includes/pop-up-block.php'; ?>
    <?php include __DIR__ . '/includes/banner-debt.php'; ?>
    <?php include __DIR__ . '/includes/services.php'; ?>
    <?php include __DIR__ . '/includes/csd.php'; ?>
    <?php include __DIR__ . '/includes/write-downs.php'; ?>
    <?php include __DIR__ . '/includes/base.php'; ?>
    <?php include __DIR__ . '/includes/debt.php'; ?>
    <?php include __DIR__ . '/includes/questions.php'; ?>
    <?php include __DIR__ . '/includes/video.php'; ?>
    <?php include __DIR__ . '/includes/consultation.php'; ?>
    <?php include __DIR__ . '/includes/footer.php'; ?>



    <script src="/assets/js/main.js"></script>

</body>

</html>